    ...
    <form action="" method="post" @submit.prevent="submit()" id="form1" novalidate>
      <div class="form-floating mb-3" id="inputpelicula">
        <select class="form-select input-group-sm has-validation" id="pelicula" aria-label="Floating label select example" v-model="pelicula" required>
          ...
          <option value="La cenicienta">La cenicienta</option>
        </select>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="(funcion, index) in funciones" :key="index">
            ...
            <td><button class="btn btn-danger" @click="EliminarFuncion(funcion)">Eliminar</button></td>
...
export default  {
    ...
    props: [],
    created(){
      axios.get('https://639a6077d514150197347436.mockapi.io/cinema/funciones')
      .then((response) => {
    ...
    data () {
      return {
        ...
        formato_funcion: '',
        funciones: [],
        urlFunciones: 'https://639a6077d514150197347436.mockapi.io/cinema/funciones'
      }
    },
    methods: {
      submit(){
        ...
        axios.post('https://639a6077d514150197347436.mockapi.io/cinema/funciones', fn)
        .then((response) =>
        {
      ...
      EliminarFuncion(el){
        ...
        const deleteURL = this.urlFunciones+"/"+idToDelete;
        axios.delete(deleteURL)
        .then((response) => {
          console.table(response.data)
          this.funciones = this.funciones.filter(e => e.id != idToDelete)
